package attestations.attestation1.repositories.impl;

import attestations.attestation1.exception.UserNotFoundException;
import attestations.attestation1.model.User;
import attestations.attestation1.repositories.UserRepository;
...
public class UserRepositoryFileImpl implements UserRepository {
    ...
    private List<User> readFile() {
        ...
        List<User> result;
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader("src/attestations/attestation1/" + FILE_NAME))) {
    ...
    private void writeUsersToFile(List<User> users) {
        try (BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter("src/attestations/attestation1/" + FILE_NAME))) {
            users.stream()
                    .map(User::toString)
                    .forEach(line -> {
                        try {
